...
<script lang="ts">
	...
	import { wallet, walletBalance, sendLNPayment, generateToken } from '$lib/client/stores/wallet';
	import { scanResult } from '$lib/client/stores/scan-store';
	import {
		LoaderCircle,
		AlertCircle,
		CheckCircle2,
	...
	import TokenQrCode from '$lib/components/qr-codes/token-qr-code.svelte';
	import {
		getBolt11Amount,
		getBolt11Description,
		getBolt11ExpiresAt
	} from '@nostr-dev-kit/ndk-wallet';
	import { copyToClipboard, pasteFromClipboard } from '$lib/utils';
	...
	async function handleSendLightning() {
		...
		try {
			...
			const result = await sendLNPayment(lnInvoice, paymentAmount);
